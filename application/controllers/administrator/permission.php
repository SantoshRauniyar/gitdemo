...
class Groups extends Template
{
	public function __construct()
	{
		...
		$this->load->model('premission_model');
		...
		$this->set_title('Permission Manager');
	...
	*/
	public function all()
	{
		...
		$sort = !isset($_REQUEST['sort'])?'group_title':$_REQUEST['sort'];
		$type = !isset($_REQUEST['type'])?'desc':$_REQUEST['type'];
		
		$userdata = $this->permission_model->getgroupslist($sort,$type);
		if($type=='asc')
			$type ='desc';
		else
			$type ='asc';
