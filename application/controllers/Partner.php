	...
	$this->load->model('user_classification_model');
		...
		if(!$this->session->userdata('user_role'))
                ...
                function partner_registration()
            {
                            ...
                            $heading='Country Partner Officer';
                            $assign_role=36;
                            $action=base_url('Partner/do_save');
                            $data['countrylist']=$this->db->select('id,country')->from('country')->order_by('country','asc')->get()->result();
	                 
                
                $data['currencylist']=$this->db->select('id,cname')->from('currency')->order_by('cname','asc')->get()->result();
                            
                $this->view('partner/add',compact('heading','assign_role','data','action'));
            }
	                  ...
	                  array(
	                     'field'   => 'country', 
	                     'label'   => 'Country', 
	                     'rules'   => 'trim|is_unique[partner.country_id]'
	                  ),
	                  array(
	                     'field'   => 'state', 
	                     'label'   => 'State', 
		...
		//	$this->session->set_flashdata('adduserdata',$data);
			redirect('Partner/partner_registration');
		}
                        ...
                        public function index($action=null)
                        {
                            ...
                            if($this->user_classification_model->check_role_auth($this->session->userdata('user_role'),'is_view_country_partner'))
                            {
                            
                            $data['main_heading']='Country Partner';
                     
                            $this->db->select('partner.id,users.email,users.status,partner.business_name,country.country,state.state,district.district_name,city.city,partner.contact_person,users.user_name');
                            $this->db->from('partner');
                            $this->db->join('country','country.id=partner.country_id','left');
                            ...
                            $this->db->where('users.user_role',36);
                           // $this->db->where('partner.created_by',$this->session->userdata('id'));
                            ...
                            return $this->view('partner/index',$data);
                            }
                            else
                            {
                        ...
                        public function edit_list()
                        {
                            ...
                            if($this->user_classification_model->check_role_auth($this->session->userdata('user_role'),'is_edit_country_partner'))
                            {
                            ...
                            public function delete_list()
                        {
                            ...
                            if($this->user_classification_model->check_role_auth($this->session->userdata('user_role'),'is_del_country_partner'))
                            {
                             ...
                             if($this->user_classification_model->check_role_auth($this->session->userdata('user_role'),'is_del_country_partner'))
                            {
                                                    ...
                                                    if($this->db->delete('partner',['id'=>$id]) and $this->db->delete('users',['id'=>$isExist[0]->user_id]))
                                        {
                                            ...
                                            return redirect('Partner/');
                                                ...
                                                return  redirect('Partner/delete_list');
                                                }
                            }
                            else
                            {
                            ...
                            $heading='Country Partner Officer Edit';
                            $assign_role=36;
                            ...
                            $data['currencylist']=$this->db->select('id,cname')->from('currency')->order_by('cname','asc')->get()->result();
                                                 ...
                                                 $data['partner']=$pt[0];
                                                 return  $this->view('partner/edit',compact('data','heading'));
			...
			redirect('Partner/partner_registration');
                       ...
                       public function view_partner($id)
                            ...
                            if($this->user_classification_model->check_role_auth($this->session->userdata('user_role'),'is_view_country_partner'))
                            {
                                                 ...
                                                 $data['partner']=$pt[0];
                                                 //var_dump($pt[0]);exit();
                                                 return  $this->view('partner/view',compact('data','heading'));
