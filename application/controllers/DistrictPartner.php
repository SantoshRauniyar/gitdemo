...
class DistrictPartner extends Template
{
	...
	public function __construct()
	{
		...
		if(!$this->session->userdata('user_role'))
                ...
                function create()
            {
                           ...
                           $cdata=$this->users_model->get_state_ByUid();
                             ...
                             $data['districtlist']=$this->db->select('id,district_name')->from('district')->where('state_id',$cdata->id)->order_by('district_name','asc')->get()->result();
                                   
                                           $action=base_url('DistrictPartner/do_save');        
                                                   
                
                
                $this->view('partner/district/add',compact('heading','assign_role','data','loc','action'));
            }
		...
		//	$this->session->set_flashdata('adduserdata',$data);
			redirect('DistrictPartner/create');
		}
                                ...
                                else
                                {
                                   if($do=$this->db->insert('users',$userCred))
                                   {
                                       ...
                                       if($this->db->insert('partner',$business_details))
                                       {
                                          ...
                                          return redirect('DistrictPartner/');
                                       }
                                       else
                                       {
                        ...
                        public function index($action=null)
                        {
                            ...
                            if($this->user_classification_model->check_role_auth($this->session->userdata('user_role'),'is_view_district_partner'))
                            {
                            
                            $data['main_heading']='District Partner';
                     
                            $this->db->select('partner.id,users.email,users.status,partner.business_name,state.state,district.district_name,city.city,partner.contact_person,users.user_name');
                            ...
                            $this->db->where('users.user_role',22);
                            ...
                            return $this->view('partner/district/index',$data);
                            }
                            else
                            {
                        ...
                        public function edit_list()
                        {
                            ...
                            if($this->user_classification_model->check_role_auth($this->session->userdata('user_role'),'is_edit_district_partner'))
                            {
                        ...
                        public function delete_list()
                        {
                            ...
                            if($this->user_classification_model->check_role_auth($this->session->userdata('user_role'),'is_del_district_partner'))
                            {
                            ...
                            if($this->user_classification_model->check_role_auth($this->session->userdata('user_role'),'is_edit_district_partner'))
                            {
                                            ...
                                            $data['statelist']=$this->db->select('id,state')->from('state')->order_by('state','asc')->get()->result();
                                                    ...
                                                    $cdata=$this->users_model->get_state_ByUid();
                                                $data['statelist']=$this->db->select('id,state')->from('state')->where('state_id',$cdata->id)->order_by('state','asc')->get()->result();
                            ...
                            $data['districtlist']=$this->db->select('id,district_name')->from('district')->where('state_id',$cdata->id)->order_by('district_name','asc')->get()->result();
                                                   ...
                                                   // var_dump($cdata);exit();
                            $data['citylist']=$this->db->select('id,city')->from('city')->where('district_id',$cdata->id)->order_by('city','asc')->get()->result();
                            ...
                            $assign_role=37;
                                                 ...
                                                 $data['partner']=$pt[0];
                                                 return  $this->view('partner/district/edit',compact('data','heading'));
			...
			redirect('DistrictPartner/create');
                         ...
                         public function view_partner($id)
                            ...
                            if($this->user_classification_model->check_role_auth($this->session->userdata('user_role'),'is_view_district_partner'))
                            {
                                                 ...
                                                 $data['partner']=$pt[0];
                                                 return  $this->view('partner/district/view',compact('data','heading'));
