			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<div class="row">
                      ...
                      <input type="hidden" value="<?= $published_data->id ?>" name="task_id">
...
<?php }  else {?>
                    ...
                    <?php } } ?>
							...
							<form role="form" name="editprofileform" id="editprofileform" method="post" action="<?= base_url('task/do_update_publish_content').'/'.$published_data->id ?>" enctype="multipart/form-data">
								...
								<div id="firstform">
           						...
           						<label>Select Content Category <strong><font color='red'>*</font></strong></label>
           										<div class="row">
           											...
           											<div class="col-md-4">
           												<select class="	form-control" name="cont_cat" <?= !empty($static_view)?"disabled=''":"" ?>>
										...
										<?php
											...
											?>
           											...
           											<div class="col-md-4">
           	<select class="	form-control" name="content_type" <?= !empty($static_view)?"disabled=''":"" ?>>
           			...
           			<?php
                                  ...
                                  <div class="col-md-4">
                                  <select class=" form-control" name="content_code" <?= !empty($static_view)?"disabled=''":"" ?>>
                    ...
                    <?php
                      ...
                      ?>
                          ...
                          <div class="form-group">
                            ...
                            <input type="text" name="task_id" value="<?= $published_data->task_id ?>" class="form-control" <?= !empty($static_view)?"readonly=''":"" ?>>
                            <span class="text-danger"><?= form_error('task_id')?></span>
                          </div>

                                                    <div class="form-group">
                            <label>Post publishing Task Name</label>
                            <input type="text" value="<?= $published_data->task_name ?>" <?= !empty($static_view)?"readonly=''":"" ?> name="task_name" class="form-control">
                            ...
                            <div class="form-group">
                              <label style="color:#ef0f0f">Instruction on creating on post ?</label>
                                                      ...
                                                      <div class="form-group">
                            <label>Include Text</label> &nbsp;&nbsp;&nbsp;<input <?= !empty($published_data->is_text)?"checked":"" ?> type="checkbox" name="is_text" class="is_text">
                            <input type="url" <?= !empty($static_view)?"readonly=''":"" ?> name="is_text" class="form-control" id="is_text" value="<?= $published_data->is_text ?>" placeholder="Google Drive Link" >
                          </div>
 


                           <div class="form-group">
                            <label>Include Video</label>&nbsp;&nbsp;&nbsp;<input <?= !empty($published_data->is_video)?"checked":"" ?> type="checkbox" name="is_video" class="is_video">
                            <input type="url" <?= !empty($static_view)?"readonly=''":"" ?> name="is_video" class="form-control" id="is_video" value="<?= $published_data->is_video ?>" placeholder="Google Drive Link" >
                          </div>

                                                     <div class="form-group">
                            <label>Include Image</label>&nbsp;&nbsp;&nbsp;<input type="checkbox" <?= !empty($published_data->is_image)?"checked":"" ?> name="is_image" class="is_image">
                            <input type="url" <?= !empty($static_view)?"readonly=''":"" ?> name="is_image" class="form-control" id="is_image" value="<?= $published_data->is_image ?>" placeholder="Google Drive Link" >
                          </div>

                                                     <div class="form-group">
                            <label>Include Keywords</label>&nbsp;&nbsp;&nbsp;<input type="checkbox"  <?= !empty($published_data->is_keyword)?"checked":"" ?> class="is_keywords">
                            <input type="url" name="is_keywords"  <?= !empty($static_view)?"readonly=''":"" ?> class="form-control" id="is_keywords" value="<?= $published_data->is_keyword ?>" placeholder="Google Drive Link" >
                          </div>
           					          
                              <div class="form-group">
                                    <label>Landing Page ?</label>
                                     <input type="radio" <?= !empty($static_view)?"readonly=''":"" ?> <?= (int)$published_data->is_landing>0?"checked":"" ?> name="is_landing">Yes <input <?= (int)$published_data->is_landing==0?"checked":"" ?> <?= !empty($static_view)?" readonly=''":"" ?> type="radio" name="is_landing"> No
                					...
                					<textarea name="task_details" rows="10" <?= !empty($static_view)?"readonly=''":"" ?> class=" form-control"><?= $published_data->task_details ?></textarea>
                    ...
                    <div class="row">
                         <div class="col-md-4">
                          <label>Select channel</label>
                           <select class="form-control" name="channel" <?= !empty($static_view)?"disabled=''":"" ?>>
                         ...
                         <div class="col-md-8">
                          <label>Select Avenue Groups</label>
                           <select class="form-control" name="group" <?= !empty($static_view)?"disabled=''":"" ?>>
                      ...
                      <br>
                        <h4 style="color:#ef0f0f"><b>Assign</b></h4>
           			    	...
           			    	<div class="form-group">
									...
									<select class="form-control" name="assign_to" <?= !empty($static_view)?"disabled=''":"" ?>>
									...
									<select class="	form-control" name="users[]" multiple="" <?= !empty($static_view)?"disabled=''":"" ?>>
												<option     value="" hidden="">Select Please</option>
											<?php
                              $follower=explode('-', $published_data->followers);
											

                        foreach ($follower as $key => $v) { 

                          foreach ($users as $value) {
                         
										?>

											<option value="<?=$value->id ?>" <?= $v==$value->id?"selected":"" ?>><?= $value->user_name ?></option>
                ...
                <div class="row">
									...
									<input type="datetime-local" <?= !empty($static_view)?"disabled=''":"" ?> value="<?= $published_data->start_date ?>" class="form-control" name="start_date">
									<span class="text-danger"><?= form_error('start_date') ?></span>
								</div>

                <div class="col-md-6" >
                  <label>Target Completion Date</label>
                  <input type="datetime-local" <?= !empty($static_view)?"disabled=''":"" ?> value="<?= $published_data->end_date ?>" class="form-control" name="end_date">
        ...
        <br>
                						<input type="submit" value="Next" class="btn btn-info pull-right" style="border-color: #ef0f0f;background-color:#ef0f0f;color:white;" name="Update">
